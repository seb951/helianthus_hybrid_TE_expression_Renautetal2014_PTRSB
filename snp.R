...
#Here we are interested in finding calls for all individuals, for all species and all individuals....
...
###########################
snp = function(list_ind= "reference/hybrid_species",user = "renaut", species = "deserticola",genes = 51468, cpu = 16,wd = "/media/seb_1TB/transposon") {
...
mpileup = paste("samtools mpileup -C50 -I -ugf reference/HA412_trinity_noAltSplice_400bpmin.fa ", all_final, " -r ",split[j,1]," | bcftools view -cvg - > mpileup2/variants",j,".raw.vcf", sep = "")
Sys.sleep(ifelse(read.table("prog") <= cpu,0, (read.table("prog") - cpu)^4    ))
mpileup_exe = paste("mpileup2/cmd2/mpileup_",j,sep = "")
...
system("echo -n '' > mpileup2/0_51k_variants.raw.vcf") #final file
system("cat mpileup2/variants*  >>mpileup2/0_51k_variants.raw.vcf") # cat verything
system("grep '#' -v  mpileup2/0_51k_variants.raw.vcf >mpileup2/0_51k_variants.clean.vcf") #tidy up
system("rm mpileup2/variants*")
system("rm mpileup2/cmd2/*")


###Pre-cleaning step###
	system("wc -l mpileup2/0_51k_variants.clean.vcf >mpileup2/wordcount1")
	wordcount = read.table("mpileup2/wordcount1")
	system("awk 'NR == 27' mpileup2/0_51k_variants.raw.vcf  >mpileup2/header")
	header = read.delim("mpileup2/header", header = F, stringsAsFactors = F)
	...
	write.table(t(as.matrix(c("reference", "position","reference_annuus_allele",header))),"results2/snp_table",row.names = F, col.names = F, quote = F, sep = " ")

	temp = NULL
	mis = NULL
	con = file("mpileup2/0_51k_variants.clean.vcf")
	open(con)
	for(i in 1:as.numeric(wordcount[1]))
	{
		...
		if((length(xxx[xxx == "XX"]) / length(xxx[4:length(xxx)])) < 0.4)	cat(t(as.matrix(xxx)),file = "results2/snp_table",append = T, fill = F, "\n") else mis = c(mis, i) #only cat loci with less than 40% missing data. 
	#	if(regexpr(xxx[3],paste(xxx[4:21],collapse = "")) < 0) temp = rbind(temp,xxx)
		if(i %% 1000 == 0) print(paste(i,"of", wordcount[1], Sys.time()))
		}
close(con)

if(species == "deserticola") system("cat results2/snp_table | sed 's/[ \t]*$//' >results2/snp_table_deserticola")
if(species == "paradoxus")system("cat results2/snp_table | sed 's/[ \t]*$//' >results2/snp_table_paradoxus")
if(species == "anomalus") system("cat results2/snp_table | sed 's/[ \t]*$//' >results2/snp_table_anomalus")
if(species == "all") system("cat results2/snp_table | sed 's/[ \t]*$//' >results2/snp_table_all")

system("rm results2/snp_table reference/temp*") ### rm snp_table and other temp files 
system("rm mpileup2/0_51* mpileup2/header mpileup2/wordcount1 log prog ") ### rm undesirable outputs
}
...
for(s in 1:3)
{
if(s == 1) {system("samtools mpileup -C50 -Iuf reference/HA412_trinity_noAltSplice_400bpmin.fa alignments/king141B.fq.sorted.bam | bcftools view -cg - | Rcode/vcfutils.pl vcf2fq > cns.fq");snp =nrow(read.table("results2/snp_table_deserticola", stringsAsFactors = F))}
if(s == 2) {system("samtools mpileup -C50 -Iuf reference/HA412_trinity_noAltSplice_400bpmin.fa alignments/Sample_Ano1506.fq.sorted.bam | bcftools view -cg - | Rcode/vcfutils.pl vcf2fq > cns.fq");snp = nrow(read.table("results2/snp_table_anomalus", stringsAsFactors = F))}
if(s == 3) {system("samtools mpileup -C50 -Iuf reference/HA412_trinity_noAltSplice_400bpmin.fa alignments/Sample_king1443.fq.sorted.bam | bcftools view -cg - | Rcode/vcfutils.pl vcf2fq > cns.fq");snp = nrow(read.table("results2/snp_table_paradoxus", stringsAsFactors = F))}
...
write.table(results,"results2/snp_stats",row.names = species, col.names = T)
...
if(fun == "snp") snp(list_ind= "reference/hybrid_species",user = "renaut",species = args[1],cpu = as.numeric(args[2]),wd = "~/transposon", genes = 30000)
